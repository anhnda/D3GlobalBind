...
def parse_arguments():
    ...
    p.add_argument('--config', type=argparse.FileType(mode='r'),
                   default='configs_clean/RDKitCoords_flexible_self_docking.yml')
...
#     t = globals()[args.model_type]
#     print(t)
#     model = t(device=device, lig_input_edge_feats_dim=data_sample[0].edata['feat'].shape[1],rec_input_edge_feats_dim=data_sample[1].edata['feat'].shape[1], **args.model_parameters, **kwargs)
#     return model
...
def main_function():
    args = get_arguments()
    from commons.func import getCurrentTimeString
    from commons.mem_watch_dog import MemWatchDog
    mem_log_path = "logs/mem_%s" % getCurrentTimeString()
    start_time = time.time()
    memWatchDog = MemWatchDog(logPath=mem_log_path, interval=5)
    memWatchDog.start_monitor(os.getpid(), start_time=start_time)
