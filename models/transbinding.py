...
from models.D3GPairTransformer import D3GPairTransformer2
...
class GPairTrans(nn.Module):
    ...
    def __init__(self, n_lays, debug, device, use_rec_atoms, shared_layers, noise_decay_rate, cross_msgs, noise_initial,
        ...
        super(GPairTrans, self).__init__()
        self.debug = debug
        self.cross_msgs = cross_msgs
        self.device = device
        self.save_trajectories = save_trajectories
        ...
        self.gpairTransformer = D3GPairTransformer2(iegmn_lay_hid_dim, n_d3graph_layer=n_d3graph_layer,
...
class TransBinding(nn.Module):

    def __init__(self, device='cuda:0', debug=False, use_evolved_lig=False, evolve_only=False, **kwargs):
        super(TransBinding, self).__init__()
        self.debug = debug
        self.evolve_only = evolve_only
        self.use_evolved_lig = use_evolved_lig
        self.device = device
        self.modelg = GPairTrans(device=self.device, debug=self.debug, evolve_only=self.evolve_only, **kwargs)
    ...
    def __repr__(self):
        return "TransBind " + str(self.__dict__)
