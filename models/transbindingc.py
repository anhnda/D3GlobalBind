...
# from models.D3GPairTransformer import D3GPairTransformer
from models.D3GPairTransformer import D3GPairTransformerC
...
class GPairTransC(nn.Module):
    ...
    def __init__(self, n_lays, debug, device, use_rec_atoms, shared_layers, noise_decay_rate, cross_msgs, noise_initial,
        ...
        super(GPairTransC, self).__init__()
        self.debug = debug
        self.cross_msgs = cross_msgs
        self.device = device
        self.save_trajectories = save_trajectories
        ...
        self.gpairTransformer = D3GPairTransformerC(iegmn_lay_hid_dim+3, n_d3graph_layer=n_d3graph_layer,
    ...
    def forward(self, lig_graph, rec_graph, geometry_graph, complex_names, epoch):
        ...
        # print("DB ")
        # print(h_feats_lig.size(0))
        # print(self.random_vec_dim)
        if self.random_vec_dim == 0:
            rand_h_lig = torch.zeros((h_feats_lig.size(0), 1)).to(self.device)
            rand_h_rec = torch.zeros((h_feats_rec.size(0), 1)).to(self.device)
        else:
        ...
        if self.batch_d3:
            ...
            for idx in range(len(ligs_node_idx) - 1):
                ...
                receptor_global_feature, ligand_global_feature, receptor_binding_centroid, ligand_binding_centroid, receptor_binding_direction, ligand_binding_direction = \
                     self.gpairTransformer(rec_feats, Z_rec_coords, lig_feats, Z_lig_coords)
                # matching_features, translation, ligand_binding_direction, receptor_binding_direction = self.gpairTransformer(rec_feats, Z_rec_coords, lig_feats, Z_lig_coords)
                ...
                ligs_keypts.append(0)
                recs_keypts.append(0)

                # aff = self.bindingAffLayer2(
                #     self.actBindingAff(
                #         self.bindingAffLayer1(torch.cat([matching_features, - matching_features]))))
                aff = self.bindingAffLayer2(
                    self.actBindingAff(
                        self.bindingAffLayer1(torch.cat([receptor_global_feature, ligand_global_feature]))))
...
class TransBindingC(nn.Module):

    def __init__(self, device='cuda:0', debug=False, use_evolved_lig=False, evolve_only=False, **kwargs):
        super(TransBindingC, self).__init__()
        self.debug = debug
        self.evolve_only = evolve_only
        self.use_evolved_lig = use_evolved_lig
        self.device = device
        self.modelg = GPairTransC(device=self.device, debug=self.debug, evolve_only=self.evolve_only, **kwargs)
    ...
    def forward(self, lig_graph, rec_graph, geometry_graph=None, complex_names=None, epoch=0):
        ...
        outputs = self.modelg(lig_graph, rec_graph, geometry_graph, complex_names, epoch)
    ...
    def __repr__(self):
        return "TransBindC " + str(self.__dict__)
