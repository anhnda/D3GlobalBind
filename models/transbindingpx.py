...
from models.D3GPairTransformer import D3GPairTransformerPX
...
class GPairTransPX(nn.Module):
    ...
    def __init__(self, n_lays, debug, device, use_rec_atoms, shared_layers, noise_decay_rate, cross_msgs, noise_initial,
        ...
        super(GPairTransPX, self).__init__()
        self.debug = debug
        self.cross_msgs = cross_msgs
        self.device = device
        self.save_trajectories = save_trajectories
        ...
        self.gpairTransformer = D3GPairTransformerPX(iegmn_lay_hid_dim + 3, n_d3graph_layer=n_d3graph_layer,
    ...
    def forward(self, lig_graph, rec_graph, geometry_graph, complex_names, epoch):
        ...
        if self.batch_d3:
            ...
            for i in range(ligand_global_features.shape[0]):
                rotation = get_rotation_matrix_from_vectors(ligand_binding_directions[i],
                                                            receptor_binding_directions[i],
                                                            device=self.device)
            ...
            for idx in range(len(ligs_node_idx) - 1):
                ...
                rotation = torch.eye(3).to(self.device)
                translation = torch.zeros(3).to(self.device)

                ligs_keypts.append(predictedPositions)
...
class TransBindingPX(nn.Module):

    def __init__(self, device='cuda:0', debug=False, use_evolved_lig=False, evolve_only=False, **kwargs):
        super(TransBindingPX, self).__init__()
        self.debug = debug
        self.evolve_only = evolve_only
        self.use_evolved_lig = use_evolved_lig
        self.device = device
        self.modelg = GPairTransPX(device=self.device, debug=self.debug, evolve_only=self.evolve_only, **kwargs)
    ...
    def forward(self, lig_graph, rec_graph, geometry_graph=None, complex_names=None, epoch=0):
        ...
        ligs_node_idx.insert(0, 0)
        for idx in range(len(ligs_node_idx) - 1):
            predicted_coords = outputs[3][idx]
    ...
    def __repr__(self):
        return "TransBindPX " + str(self.__dict__)
